...
public class GetGroups : CommonTest
{
    ...
    private async Task SeedDataForPageTests()
    {
        ...
        var groups = Fixture.CreateMany<Domain.Entities.Group>(12);

        await Context.Groups.AddRangeAsync(groups);
        await Context.SaveChangesAsync();
    }

    private async Task<(Domain.Entities.Group, Domain.Entities.Group)> SeedDataForFiltrationTests()
    {
        ClearDataBase();
    ...
    private async Task<PaginationList<Domain.Entities.Group>> Action(GetGroupsQuery query)
    {
