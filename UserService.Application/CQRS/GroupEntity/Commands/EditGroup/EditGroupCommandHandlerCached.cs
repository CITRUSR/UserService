...
public class EditGroupCommandHandlerCached(EditGroupCommandHandler handler, ICacheService cacheService)
    : IRequestHandler<EditGroupCommand, Group>
{
    ...
    public async Task<Group> Handle(EditGroupCommand request, CancellationToken cancellationToken)
    {
        ...
        await _cacheService.SetObjectAsync(CacheKeys.ById<Group, int>(request.Id), group, cancellationToken);

        await _cacheService.RemovePagesWithObjectAsync<Group, int>(group.Id, (group1, i) => group1.Id == i,
            cancellationToken);
